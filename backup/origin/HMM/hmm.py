...
def draw_result(dates, closes, lss, model):
    sns.set_style('white')
    plt.figure()
...
# 训练集模拟收益率曲线
# @update: 12.5 支持14种状态不同颜色显示
def draw_back(dates, lr, lss, model):
...
def main():
    ...
    # train
    train_X, train_logreturn = features.gen_attrs(index_hist_train_df, maxperiod)   # 生成属性
    # test
    test_X, test_logreturn = features.gen_attrs(index_hist_test_df, maxperiod)  # 生成属性
    # train hmm model
    ...
    draw_result(train_datelist, train_closeidx, latent_states_sequence_train, hmm)  # 训练集状态图
    draw_back(train_datelist, train_logreturn, latent_states_sequence_train, hmm)   # 训练集收益曲线
    ...
    draw_result(test_datelist, test_closeidx, latent_states_sequence_test, hmm)     # 测试集状态图
    selected_states = draw_back(test_datelist, test_logreturn, latent_states_sequence_test, hmm)      # 测试集收益曲线
    ...
    features.gen_attrs(dao.get_local_bar_data(code, '2010-01-01', '2017-12-31'), 10)
